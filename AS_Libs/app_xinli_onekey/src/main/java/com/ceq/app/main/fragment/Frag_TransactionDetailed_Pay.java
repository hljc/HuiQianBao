...
import static com.ceq.app.main.activity.Act_TransactionDetailed.Extra_Bean_TransactionDetailed_Pay;
...
public class Frag_TransactionDetailed_Pay extends Framework_Fragment {
    ...
    List<Bean_TransactionDetailed_Pay> bean_transactionDetailedList = new ArrayList<>();
    ...
    public void initAdapter() {
        rv_transactionDetailed.setAdapter(rva_transactionDetailed = new RecyclerView.Adapter() {
            ...
            public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
                ...
                final Bean_TransactionDetailed_Pay bean = bean_transactionDetailedList.get(position);
                ...
                if (bean.getAmount() == null)
                    mvh.tv_right.setText("-");
                else
                    mvh.tv_right.setText(new StringBuilder("￥").append(decimalFormat.format(new BigDecimal(bean.getAmount()))));

                mvh.rl_bg.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        startActivity(new Intent(getActivity(), Act_TransactionDetailed.class).putExtra(Extra_Bean_TransactionDetailed_Pay, bean));
                    }
                });
            }
        ...
        xrv.setOnRefreshHttpListener(0, bean_transactionDetailedList, rva_transactionDetailed, new View_XRefreshLayout.OnRefreshHttpListener() {
            ...
            public void onHttpStart(int currentPagePosition, View_XRefreshStatusView view_xRefreshStatusView, List list, RecyclerView.Adapter adapter) {
                ((Act_Mine_TransactionDetailed) getActivity()).getTransactionDetailedData(view_xRefreshStatusView, Constant_Api.URL_GET_DETAILED_TRANSACTION_POST, currentPagePosition, list, 1);
            }
        });
    }
