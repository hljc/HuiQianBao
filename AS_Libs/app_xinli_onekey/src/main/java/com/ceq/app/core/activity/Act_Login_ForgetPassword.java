 ...
 */
public class Act_Login_ForgetPassword extends Framework_Activity {
    ...
    public void initView() {
        //标题栏
        util_init.initTextView(R.id.icd_title, R.id.tv_title, null, getIntent().getStringExtra(Extra_String_Title), View.VISIBLE);
        ...
        //输入密码
        et_password = util_init.initEditText(R.id.icd_setPassword, R.id.et_input, "请输入新的登录密码", null, InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_PASSWORD, LENGTH_PASSWORD, View.VISIBLE);
        //再次输入密码
        et_password2 = util_init.initEditText(R.id.icd_setPassword2, R.id.et_input, "请再次输入新的登录密码", null, InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_PASSWORD, LENGTH_PASSWORD, View.VISIBLE);
        //完成
    ...
    public void onClick(View v) {
        if (v.getId() == tv_commit.getId()) {
            ...
            if (Util_Empty.isEmptyToToast(et_identify.getText().toString(), Constant_International.error_code_not_null))
                return;
            if (Util_Empty.isEmptyAndMatchToToast(et_password.getText().toString(), Constant_International.error_password_not_null, expression_password, Constant_International.error_expression_password))
                return;
            if (Util_Empty.isEmptyAndMatchToToast(et_password2.getText().toString(), Constant_International.error_password_not_null, expression_password, Constant_International.error_expression_password))
                return;
            if (!et_password.getText().toString().equals(et_password2.getText().toString())) {
                Util_Toast.toast("2次密码不一致");
                return;
            }
            forgetPasswordByHttp(et_mobile.getText().toString(), et_password.getText().toString(), et_identify.getText().toString());
    ...
    private void forgetPasswordByHttp(final String phone, final String password, final String vericode) {
        Util_Http.httpToRequest(getActivity(),Constant_Api.URL_ACCOUNT_MODIFY_PASSWORD_POST, Util_Http.RequestMethod.POST, new Util_Http.HttpHeadersAndParams() {
            @Override
