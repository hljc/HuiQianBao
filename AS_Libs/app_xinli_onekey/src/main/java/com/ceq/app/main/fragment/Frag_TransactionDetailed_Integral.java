...
import static com.ceq.app.main.activity.Act_TransactionDetailed.Extra_Bean_TransactionDetailed_Integral;

/**
 * Created by ceq on 2017/4/15.
 */

public class Frag_TransactionDetailed_Integral extends Framework_Fragment {
    RecyclerView rv_transactionDetailed;
    RecyclerView.Adapter rva_transactionDetailed;
    View_XRefreshLayout xrv;

    List<Bean_TransactionDetailed_Integral> bean_transactionDetailedList = new ArrayList<>();
    ...
    public void initAdapter() {
        rv_transactionDetailed.setAdapter(rva_transactionDetailed = new RecyclerView.Adapter() {
            ...
            public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
                ...
                final Bean_TransactionDetailed_Integral bean = bean_transactionDetailedList.get(position);
                mvh.tv_name.setText(bean.getRemark());
                mvh.tv_subName.setText("时间:".concat(bean.getCreateTime()));
                mvh.tv_right.setText(new StringBuilder(bean.getType().equals("0") ? "+" : "-").append(bean.getCoin()).append("分"));

                mvh.rl_bg.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        startActivity(new Intent(getActivity(), Act_TransactionDetailed.class).putExtra(Extra_Bean_TransactionDetailed_Integral, bean));
                    }
                });
            }
        ...
        xrv.setOnRefreshHttpListener(0, bean_transactionDetailedList, rva_transactionDetailed, new View_XRefreshLayout.OnRefreshHttpListener() {
            ...
            public void onHttpStart(int currentPagePosition, View_XRefreshStatusView view_xRefreshStatusView, List list, RecyclerView.Adapter adapter) {
                ((Act_Mine_TransactionDetailed) getActivity()).getTransactionDetailedData(view_xRefreshStatusView,Constant_Api.URL_GET_DETAILED_INTEGRAL_POST, currentPagePosition,list,  -2);
            }
        });
    }
