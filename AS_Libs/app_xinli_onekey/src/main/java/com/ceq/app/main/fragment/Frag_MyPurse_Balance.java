...
import static com.ceq.app.core.constants.Constant_Api.URL_GET_USER_BALANCE_POST;
...
public class Frag_MyPurse_Balance extends Framework_Fragment {
    RecyclerView rv_myPurse;
    RecyclerView.Adapter rva_myPurse;

    List<Bean_Purse_Balance> bean_purseBalanceList = new ArrayList<>();

    @Nullable
    @Override
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        return init(R.layout.frag_mypurse_balance);
    }
    ...
    public void initAdapter() {
        rv_myPurse.setAdapter(rva_myPurse = new RecyclerView.Adapter() {
            ...
            public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
                ...
                final Bean_Purse_Balance bean = bean_purseBalanceList.get(position);
                mvh.tv_name.setText("余额:".concat((String) Util_Empty.isEmptyToReplace(bean.getCurBal(), "0.00")).concat("元"));
                mvh.tv_name.setMaxWidth(w);
                mvh.tv_subName.setText("时间:".concat(bean.getCreateTime()));
                DecimalFormat decimalFormat=new DecimalFormat("0.00");
                decimalFormat.setRoundingMode(RoundingMode.DOWN);
                mvh.tv_right.setText((bean.getAddOrSub().equals("0") ? "+" : "-").concat( decimalFormat.format(new BigDecimal(bean.getAmount()))).concat("元"));

            }

            @Override
            public int getItemCount() {
                return bean_purseBalanceList.size();
            }
        });

        final View_XRefreshLayout xrv = (View_XRefreshLayout) findViewById(R.id.xrv);
        xrv.setOnRefreshHttpListener(0, bean_purseBalanceList, rva_myPurse, new View_XRefreshLayout.OnRefreshHttpListener() {
            @Override
            public void onHttpStart(int currentPagePosition, View_XRefreshStatusView view_xRefreshStatusView, List list, RecyclerView.Adapter adapter) {
                ((Act_MyPurse_PurseDetailed) getActivity()).getScoreBalanceShareMoneyData(view_xRefreshStatusView, URL_GET_USER_BALANCE_POST, currentPagePosition, list, 1);
            }
        });
    }
